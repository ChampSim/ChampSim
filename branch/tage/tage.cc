...
private:
  ...
  struct tage_predictor_table_entry predictor_table[TAGE_NUM_COMPONENTS][(1 << TAGE_MAX_INDEX_BITS)]; // 1 << 12 is 4096, expt with 1024 for now
...
void Tage::init()
{
  ...
  */
  ...
  for (int i = 0; i < TAGE_NUM_COMPONENTS; i++) {
    ...
    std::cout << "component : " << i << " History length: " << component_history_lengths[i] << std::endl;
