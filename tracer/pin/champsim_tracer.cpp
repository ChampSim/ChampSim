...
VOID foundByteCode(T readValue, ADDRINT PC, ADDRINT readAddr, UINT32 readSize, BOOL bytecode)
{
  if (PIN_SafeCopy(readValue, reinterpret_cast<void*>(readAddr), readSize) == readSize) {
    UINT64 loaded_val;
    if (readSize == 1) loaded_val = static_cast<UINT64>(*readValue & 0xFF);
    else if (readSize == 2) loaded_val = static_cast<UINT64>(*readValue & 0xFFFF);
    else loaded_val = static_cast<UINT64>(*readValue);
...
VOID Instruction(INS ins, VOID* v)
{
  ...
  if (INS_IsMemoryRead(ins) && (INS_Address(ins) < mainModuleHigh && INS_Address(ins) >= mainModuleBase)) {
    static const std::vector<ADDRINT> byteCodeLoadAddresses = {
        0x25c168, 0x25c444, 0x25c68e, 0x25c2c6, 0x25c58a, 0x264340, 0x2645f2, 0x264897, 0x26c661, 0x26c72d, 0x25bbf5, // NEXTOPARG()
    ...
    if (std::find_if(byteCodeLoadAddresses.begin(), byteCodeLoadAddresses.end(),
      ...
      if (!startTracing) 
        startTracing = true;
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)ByteCodeLoad, IARG_INST_PTR, IARG_MEMORYREAD_EA, IARG_UINT32, INS_MemoryOperandSize(ins, 0), IARG_BOOL, true, IARG_END);
    } else if (std::find_if(dispatchTableLoadAddresses.begin(), dispatchTableLoadAddresses.end(),
                            [insAddr](ADDRINT byteCodeLoadAddress) { return byteCodeLoadAddress == insAddr; })
               != dispatchTableLoadAddresses.end()) {
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)ByteCodeLoad, IARG_INST_PTR, IARG_MEMORYREAD_EA, IARG_UINT32, INS_MemoryOperandSize(ins, 0), IARG_BOOL, false, IARG_END);
    } else {
...
static VOID MonitorExecution(VOID* arg) {
  while (monitor) {
    ...
    getPipeStatus();
    std::cout << "\033[2K\r" 
      << "Current value of instructions seen: " << std::dec << seenInstructions 
      << " seen bytecodes: " << seenBytecodes << "    " 
      << std::flush;
  }
}
...
VOID Fini(INT32 code, VOID* v)
{
  ...
  std::cout << "Execution time: " << std::setprecision(5) << ((double) std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count())/1000.0 << " seconds \n";
}
