      ...
      if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
        ...
        for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
          ...
          if (arch_instr.destination_memory[i]) {
            ...
            // instructions properly without deadlock
            if (num_mem_ops > 0) {
#ifdef SANITY_CHECK
...
void O3_CPU::execute_instruction() {
  ...
  while (exec_issued < EXEC_WIDTH) {
    if (RTE0[RTE0_head] < ROB_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTE0] is empty head: " << RTE0_head << " tail: " << RTE0_tail
      // << endl; });
      break;
    }
  ...
  while (exec_issued < EXEC_WIDTH) {
    if (RTE1[RTE1_head] < ROB_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTE1] is empty head: " << RTE1_head << " tail: " << RTE1_tail
      // << endl; });
      break;
    }
  ...
  if (not_available == 0) {
    ...
    DP(if (warmup_complete[cpu]) {
      cout << "[ROB] " << __func__
           << " instr_id: " << ROB.entry[rob_index].instr_id
           << " rob_index: " << rob_index;
      cout << " scheduled all num_mem_ops: " << ROB.entry[rob_index].num_mem_ops
           << endl;
    });
  }
      ...
      if (ROB.entry[rob_index].destination_added[i])
        ...
        DP(if (warmup_complete[cpu]) {
          ...
          cout << " cannot be added in the store queue occupancy: "
               << SQ.occupancy << " cycle: " << current_core_cycle[cpu] << endl;
        });
      }
    }
  }
    ...
    if (SQ.entry[i].virtual_address ==
      ...
      if ((LQ.entry[lq_index].producer_id == UINT64_MAX) &&
        ...
        DP(if (warmup_complete[cpu]) {
          ...
          cout << " to obey WAR store instr_id: " << SQ.entry[i].instr_id
  ...
  if (forwarding_index != SQ.SIZE) { // we have a store-to-load forwarding
    ...
    if ((SQ.entry[forwarding_index].fetched == COMPLETED) &&
      ...
      LQ.entry[lq_index].physical_address =
          ...
          (LQ.entry[lq_index].virtual_address & ((1 << LOG2_BLOCK_SIZE) - 1));
    ...
    if (ROB.entry[prior].destination_memory[i] ==
      ...
      DP(if (warmup_complete[cpu]) {
        cout << "[LQ] " << __func__
             ...
             << " lq_index: " << lq_index;
        cout << hex << " address: " << ROB.entry[prior].destination_memory[i]
             << dec << endl;
      });
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index) {
  ...
  */
  ...
  DP(if (warmup_complete[cpu]) {
    ...
    cout << " is added in the SQ translated: " << +SQ.entry[sq_index].translated
...
void O3_CPU::operate_lsq() {
  ...
  while (store_issued < SQ_WIDTH) {
    if (RTS0[RTS0_head] < SQ_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTS0] is empty head: " << RTS0_head << " tail: " << RTS0_tail
      // << endl; });
      break;
    }
  ...
  while (store_issued < SQ_WIDTH) {
    if (RTS1[RTS1_head] < SQ_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTS1] is empty head: " << RTS1_head << " tail: " << RTS1_tail
      // << endl; });
      break;
    }
  ...
  while (load_issued < LQ_WIDTH) {
    if (RTL0[RTL0_head] < LQ_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTL0] is empty head: " << RTL0_head << " tail: " << RTL0_tail
      // << endl; });
      break;
    }
  ...
  while (load_issued < LQ_WIDTH) {
    if (RTL1[RTL1_head] < LQ_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[RTL1] is empty head: " << RTL1_head << " tail: " << RTL1_tail
      // << endl; });
      break;
    }
...
void O3_CPU::execute_store(uint32_t rob_index, uint32_t sq_index,
  ...
  DP(if (warmup_complete[cpu]) {
    cout << "[SQ1] " << __func__ << " instr_id: " << SQ.entry[sq_index].instr_id
         << hex;
        ...
        if (ROB.entry[dependent].source_memory[j] &&
          ...
          if (ROB.entry[dependent].source_memory[j] ==
            ...
            DP(if (warmup_complete[cpu]) {
              ...
              cout << SQ.entry[sq_index].instr_id << " remain_num_ops: "
                   << ROB.entry[fwr_rob_index].num_mem_ops
                   << " cycle: " << current_core_cycle[cpu] << endl;
            });
      ...
      if (ROB.entry[rob_index].registers_index_depend_on_me[j].search(i)) {
        ...
        DP(if (warmup_complete[cpu]) {
          ...
          cout << ROB.entry[i].instr_id
               ...
               << " num_reg_dependent: " << ROB.entry[i].num_reg_dependent
               << " cycle: " << current_core_cycle[cpu] << endl;
        });
      }
    }
  ...
  if (is_it_tlb) { // DTLB
    ...
    if (queue->entry[index].type == RFO) {
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " full_addr: " << SQ.entry[sq_index].physical_address << dec
...
void O3_CPU::handle_o3_fetch(PACKET *current_packet, uint32_t cache_type) {
  ...
  if (cache_type == 0) { // DTLB
    ...
    if (current_packet->type == RFO) {
      SQ.entry[sq_index].physical_address =
          (current_packet->data_pa << LOG2_PAGE_SIZE) |
          (SQ.entry[sq_index].virtual_address &
           ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
      SQ.entry[sq_index].translated = COMPLETED;
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " DTLB_FETCH_DONE translation: "
             ...
             << (SQ.entry[sq_index].physical_address >> LOG2_PAGE_SIZE);
        cout << " full_addr: " << SQ.entry[sq_index].physical_address << dec
             << " store_merged: " << +current_packet->store_merged;
    ...
    if (current_packet->type == RFO)
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec
             << " remain_mem_ops: " << ROB.entry[rob_index].num_mem_ops;
        cout << " load_merged: " << +current_packet->load_merged
             << " inflight_mem: " << inflight_mem_executions << endl;
      });
  ...
  if (provider->store_merged) {
    ITERATE_SET(merged, provider->sq_index_depend_on_me, SQ.SIZE) {
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " DTLB_FETCH_DONE translation: " << +SQ.entry[merged].translated
             << hex << " page: "
             << (SQ.entry[merged].physical_address >> LOG2_PAGE_SIZE);
        cout << " full_addr: " << SQ.entry[merged].physical_address << dec
             << " by instr_id: " << +provider->instr_id << endl;
      });
    }
  }
  if (provider->load_merged) {
    ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " DTLB_FETCH_DONE translation: " << +LQ.entry[merged].translated
             << hex << " page: "
...
void O3_CPU::handle_merged_load(PACKET *provider) {
  ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
    ...
    DP(if (warmup_complete[cpu]) {
      ...
      cout << " remain_mem_ops: " << ROB.entry[merged_rob_index].num_mem_ops
           << endl;
    });
...
void O3_CPU::release_load_queue(uint32_t lq_index) {
  ...
  DP(if (warmup_complete[cpu]) {
    ...
    cout << hex << " full_addr: " << LQ.entry[lq_index].physical_address << dec
         << endl;
  });
