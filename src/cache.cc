...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET &handle_pkt)
{
    ...
    // update prefetcher
    if (cache_type == IS_L1I)
        l1i_prefetcher_cache_fill(handle_pkt.cpu, (virtual_prefetch ? handle_pkt.full_v_addr : handle_pkt.full_addr) & ~(BLOCK_SIZE-1), set, way, handle_pkt.type == PREFETCH, evicting_address & ~(BLOCK_SIZE-1));
    if (cache_type == IS_L1D)
