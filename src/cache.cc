...
CACHE::mshr_type::mshr_type(const tag_lookup_type& req, champsim::chrono::clock::time_point _time_enqueued)
    : address(req.address), v_address(req.v_address), ip(req.ip), instr_id(req.instr_id), cpu(req.cpu), type(req.type),
      clusivity(req.clusivity), prefetch_from_this(req.prefetch_from_this), time_enqueued(_time_enqueued), instr_depend_on_me(req.instr_depend_on_me),
...
CACHE::mshr_type CACHE::mshr_type::merge(mshr_type predecessor, mshr_type successor)
{
  ...
  std::vector<channel_type*> merged_return{};
...
auto CACHE::in_set_finder(champsim::address addr) const
{
  return [matcher=matches_address(addr)](const auto& entry) {
    return entry.valid && matcher(entry);
  };
}
...
long CACHE::operate()
{
  ...
  auto has_data = [time = current_time](const auto& x) { return x.data_promise.is_ready_at(time); };
  for (auto q : {std::ref(MSHR), std::ref(inflight_writes)}) {
    auto [fill_begin, fill_end] = champsim::get_span_p(std::cbegin(q.get()), std::cend(q.get()), fill_bw, has_data);
...
uint64_t CACHE::get_way(uint64_t address, uint64_t /*unused set index*/) const
{
  ...
  return static_cast<uint64_t>(std::distance(begin, std::find_if(begin, end, in_set_finder(intern_addr))));
}
// LCOV_EXCL_STOP
...
long CACHE::invalidate_entry(champsim::address inval_addr)
{
  ...
  auto inv_way = std::find_if(begin, end, in_set_finder(inval_addr));
...
{
  if constexpr (champsim::debug_print) {
    fmt::print("[{}] {} address: {} v_address: {} set: {} cycle: {}\n", NAME, __func__, inval_block.address, inval_block.v_address,
               get_set_index(inval_block.address), current_time.time_since_epoch() / clock_period);
  }
