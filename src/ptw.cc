...
PageTableWalker::PageTableWalker(string v1, uint32_t cpu, unsigned fill_level,
                                 uint32_t v2, uint32_t v3, uint32_t v4,
                                 uint32_t v5, uint32_t v6, uint32_t v7,
                                 uint32_t v8, uint32_t v9, uint32_t v10,
                                 uint32_t v11, uint32_t v12, uint32_t v13,
                                 unsigned latency, MemoryRequestConsumer *ll)
    : champsim::operable(1), MemoryRequestConsumer(fill_level),
      MemoryRequestProducer(ll), NAME(v1), cpu(cpu), MSHR_SIZE(v11),
      MAX_READ(v12),
      MAX_FILL(v13), RQ{v10, latency}, PSCL5{"PSCL5", 4, v2,
                                             v3}, // Translation from L5->L4
  ...
  while (reads_this_cycle > 0 && RQ.has_ready() &&
    ...
    DP(if (warmup_complete[packet->cpu]) {
      std::cout << "[" << NAME << "] " << __func__
                << " instr_id: " << handle_pkt.instr_id;
      std::cout << " address: " << std::hex
                << (handle_pkt.address >> LOG2_PAGE_SIZE)
      ...
      std::cout << " event: " << handle_pkt.event_cycle
                << " current: " << current_cycle << std::endl;
    });

    auto ptw_addr = splice_bits(
        CR3_addr,
        vmem.get_offset(handle_pkt.address, vmem.pt_levels - 1) * PTE_BYTES,
        LOG2_PAGE_SIZE);
    auto ptw_level = vmem.pt_levels - 1;
    for (auto pscl : {&PSCL5, &PSCL4, &PSCL3, &PSCL2}) {
      if (auto check_addr = pscl->check_hit(handle_pkt.address);
          check_addr.has_value()) {
    ...
    packet.fill_level =
        lower_level->fill_level; // This packet will be sent from L1 to PTW.
...
void PageTableWalker::handle_fill() {
  ...
  while (fill_this_cycle > 0 && !std::empty(MSHR) &&
         MSHR.front().event_cycle <= current_cycle) {
    ...
    if (fill_mshr->translation_level == 0) // If translation complete
    {
      ...
      if (warmup_complete[cpu] && fault) {
        ...
        DP(if (warmup_complete[packet->cpu]) {
          std::cout << "[" << NAME << "] " << __func__
                    << " instr_id: " << fill_mshr->instr_id;
          std::cout << " address: " << std::hex
                    << (fill_mshr->address >> LOG2_PAGE_SIZE)
                    << " full_addr: " << fill_mshr->address;
          ...
          std::cout << " index: " << std::distance(MSHR.begin(), fill_mshr)
                    << " occupancy: " << get_occupancy(0, 0);
          std::cout << " event: " << fill_mshr->event_cycle
                    << " current: " << current_cycle << std::endl;
        });
      ...
      auto [addr, fault] = vmem.get_pte_pa(cpu, fill_mshr->v_address,
...
int PageTableWalker::add_rq(PACKET *packet) {
  ...
  auto found_rq = std::find_if(
      RQ.begin(), RQ.end(), eq_addr<PACKET>(packet->address, LOG2_PAGE_SIZE));
...
void PagingStructureCache::fill_cache(uint64_t next_level_paddr,
                                      uint64_t vaddr) {
  auto set_idx = (vaddr >> vmem.shamt(level + 1)) & bitmask(lg2(NUM_SET));
  auto set_begin = std::next(std::begin(block), set_idx * NUM_WAY);
  auto set_end = std::next(set_begin, NUM_WAY);
  auto fill_block =
      std::max_element(set_begin, set_end, lru_comparator<block_t, block_t>());
...
std::optional<uint64_t> PagingStructureCache::check_hit(uint64_t address) {
  ...
  auto hit_block = std::find_if(
      set_begin, set_end, eq_addr<block_t>{address, vmem.shamt(level + 1)});
  ...
  if (!std::empty(MSHR)) {
    ...
    for (PACKET entry : MSHR) {
      ...
      std::cout << " address: " << std::hex << entry.address
                << " v_address: " << entry.v_address << std::dec
                << " type: " << +entry.type;
