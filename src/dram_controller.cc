  ...
  for (uint32_t i = 0; i < queue->SIZE; i++) {
    if (queue->entry[i].scheduled) {
      ...
      uint32_t op_cpu = queue->entry[i].cpu,
               ...
               op_bank = dram_get_bank(op_addr), op_row = dram_get_row(op_addr);
...
void MEMORY_CONTROLLER::operate() {
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    ...
    if ((write_mode[i] == 0) && (RQ[i].next_schedule_index < RQ[i].SIZE)) {
      if (RQ[i].next_schedule_cycle <=
          current_core_cycle[RQ[i].entry[RQ[i].next_schedule_index].cpu])
        schedule(&RQ[i]);
    }

    // process DRAM requests
    if ((write_mode[i] == 0) && (RQ[i].next_process_index < RQ[i].SIZE)) {
      if (RQ[i].next_process_cycle <=
...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue) {
  ...
  for (uint32_t i = 0; i < queue->SIZE; i++) {
    ...
    if (bank_request[read_channel][read_rank][read_bank]
            .working) { // should we check this or not? how do we know if bank
                        // is busy or not for all requests in the queue?

      // DP ( if (warmup_complete[0]) {
      // cout << queue->NAME << " " << __func__ << " instr_id: " <<
      // queue->entry[i].instr_id << " bank is busy"; cout << " swrites: " <<
      // scheduled_writes[channel] << " sreads: " << scheduled_reads[channel];
      // cout << " write: " <<
      // +bank_request[read_channel][read_rank][read_bank].is_write << " read: "
      // << +bank_request[read_channel][read_rank][read_bank].is_read << hex;
      // cout << " address: " << queue->entry[i].address << dec << " channel: "
      // << read_channel << " rank: " << read_rank << " bank: " << read_bank <<
      // endl; });
    ...
    if (bank_request[read_channel][read_rank][read_bank].open_row != read_row) {
      ...
      scheduled_writes[channel] << " sreads: " << scheduled_reads[channel]; cout
      << " write: " <<
      +bank_request[read_channel][read_rank][read_bank].is_write << " read: " <<
      +bank_request[read_channel][read_rank][read_bank].is_read << hex; cout <<
      " address: " << queue->entry[i].address << dec << " channel: " <<
      read_channel << " rank: " << read_rank << " bank: " << read_bank << endl;
      });
      */
  ...
  // at this point, the scheduler knows which bank to access and if the request
  // is a row buffer hit or miss
  if (oldest_index != -1) { // scheduler might not find anything if all requests
    ...
    DP(if (warmup_complete[op_cpu]) {
      ...
      cout << " row buffer: "
           << (row_buffer_hit
                   ? (int)bank_request[op_channel][op_rank][op_bank].open_row
                   : -1)
           << hex;
      cout << " address: " << queue->entry[oldest_index].address
           << " full_addr: " << queue->entry[oldest_index].full_addr << dec;
      ...
      cout << " current: " << current_core_cycle[op_cpu]
           << " event: " << queue->entry[oldest_index].event_cycle << endl;
    });
  }
}
...
void MEMORY_CONTROLLER::process(PACKET_QUEUE *queue) {
  ...
  uint32_t op_cpu = queue->entry[request_index].cpu,
           ...
           op_rank = dram_get_rank(op_addr), op_bank = dram_get_bank(op_addr);
#ifdef DEBUG_PRINT
  uint32_t op_row = dram_get_row(op_addr), op_column = dram_get_column(op_addr);
#endif
  ...
  if (bank_request[op_channel][op_rank][op_bank].cycle_available <=
    ...
    if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
      ...
      if (queue->is_WQ) {
        ...
        DP(if (warmup_complete[op_cpu]) {
          ...
          cout << " current_cycle: " << current_core_cycle[op_cpu]
               << " event_cycle: " << queue->entry[request_index].event_cycle
               << endl;
        });
      ...
      dbus_congested[bank_request[op_channel][op_rank][op_bank].working_type]
                    [NUM_TYPES]++;
      dbus_congested[bank_request[op_channel][op_rank][op_bank].working_type]
                    [op_type]++;

      DP(if (warmup_complete[op_cpu]) {
        cout << "[" << queue->NAME << "] " << __func__ << " dbus_occupied"
             << hex;
        cout << " address: " << queue->entry[request_index].address
             << " full_addr: " << queue->entry[request_index].full_addr << dec;
        cout << " occupancy: " << queue->occupancy << " channel: " << op_channel
             << " rank: " << op_rank << " bank: " << op_bank;
        cout << " row: " << op_row << " column: " << op_column;
        cout << " current_cycle: " << current_core_cycle[op_cpu]
             << " event_cycle: "
             << bank_request[op_channel][op_rank][op_bank].cycle_available
    ...
    if (RQ[channel].entry[index].address == 0) {
      ...
      DP(if (warmup_complete[packet->cpu]) {
        ...
        cout << " full_addr: " << packet->full_addr << dec
             << " ch: " << channel;
        cout << " rank: " << rank << " bank: " << bank << " row: " << row
             << " col: " << column;
  ...
  if (min_index < queue->SIZE) {
    ...
    DP(if (warmup_complete[queue->entry[min_index].cpu]) {
      ...
      cout << " event: " << queue->entry[min_index].event_cycle
           << " current: " << current_core_cycle[queue->entry[min_index].cpu]
...
int MEMORY_CONTROLLER::check_dram_queue(PACKET_QUEUE *queue, PACKET *packet) {
  ...
  DP(if (warmup_complete[packet->cpu]) {
    cout << "[" << queue->NAME << "] " << __func__ << " new address: " << hex
         << packet->address;
