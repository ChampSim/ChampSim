  ...
  GIVEN("A 5-level virtual memory") {
    ...
    WHEN("The PTW receives a request") {
      ...
      THEN("The PSCLs contain the request's address") {
        REQUIRE(uut.pscl.at(0).check_hit({test.asid, test.v_address, 0}).has_value());
        REQUIRE(uut.pscl.at(1).check_hit({test.asid, test.v_address, 0}).has_value());
        REQUIRE(uut.pscl.at(2).check_hit({test.asid, test.v_address, 0}).has_value());
        REQUIRE(uut.pscl.at(3).check_hit({test.asid, test.v_address, 0}).has_value());
      }
    }
  }
}
