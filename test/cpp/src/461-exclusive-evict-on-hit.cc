...
SCENARIO("An exclusive load causes an eviction") {
  GIVEN("A cache with a single filled entry") {
    ...
    THEN("This writeback is received") {
    ...
    WHEN("A exclusive load is issued") {
      ...
      AND_WHEN("The first request is re-issued") {
        decltype(mock_ul_test)::request_type test = seed_a;
        test.instr_id = id++;
        ...
        THEN("The re-issue misses") {
          REQUIRE(mock_ul_test.packets.back().return_time == mock_ul_test.packets.back().issue_time + (fill_latency + miss_latency + hit_latency + 2)); // +1 due to ordering of elements
        }
      }
    }
  }
