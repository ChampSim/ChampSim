...
ooo_model_instr champsim::test::instruction_with_memory(std::array<uint64_t, NUM_INSTR_DESTINATIONS> dmem, std::array<uint64_t, NUM_INSTR_SOURCES> smem)
{
  input_instr i;
  i.ip = 1;
  i.is_branch = false;
  ...
  std::copy(std::begin(dmem), std::end(dmem), std::begin(i.destination_memory));
  std::copy(std::begin(smem), std::end(smem), std::begin(i.source_memory));
  return ooo_model_instr{0, i};
}
