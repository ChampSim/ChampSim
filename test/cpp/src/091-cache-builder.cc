...
{
  auto [num_sets, expected_bandwidth] =
      GENERATE(std::pair{16u, 1}, std::pair{64u, 1}, std::pair{1024u, 2}, std::pair{4u * 1024u, 8}, std::pair{32u * 1024u, 64});
  champsim::cache_builder buildA{};
  buildA.sets(num_sets);
...
{
  auto [size, hit_latency, fill_latency] =
      GENERATE(std::tuple{champsim::data::kibibytes{32}, 2ull, 2ull}, std::tuple{champsim::data::kibibytes{512}, 4ull, 5ull},
