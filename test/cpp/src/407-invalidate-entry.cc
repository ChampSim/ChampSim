...
SCENARIO("A cache invalidates an entry in the upper levels of its upper levels when it receives an RFO") {
  GIVEN("A second-level cache with one filled entry") {
    constexpr uint64_t hit_latency = 4;
    constexpr uint64_t miss_latency = 3;
    constexpr uint64_t fill_latency = 2;
    ...
    champsim::channel seedL1_to_L2{};
    champsim::channel seedL2_to_ll{};
    champsim::channel test_to_ll{};

    CACHE seedL1_ul{CACHE::Builder{champsim::defaults::default_l1d}
      .name("407b-seedL1")
      .upper_levels({&mock_ul_for_seed.queues})
      .lower_level(&seedL1_to_L2)
      .hit_latency(hit_latency)
      .fill_latency(fill_latency)
    };

    CACHE seedL2_ul{CACHE::Builder{champsim::defaults::default_l1d}
      .name("407b-seedL2")
      .upper_levels({&seedL1_to_L2})
      .lower_level(&seedL2_to_ll)
      .hit_latency(hit_latency)
      .fill_latency(fill_latency)
    };
    ...
    CACHE test_ll{CACHE::Builder{champsim::defaults::default_l2c}
      .name("407b-ll")
      .upper_levels({&seedL2_to_ll, &test_to_ll})
      .lower_level(&mock_ll.queues)
      .hit_latency(miss_latency)
    };

    std::array<champsim::operable*, 7> elements{{&seedL1_ul, &seedL2_ul, &test_ul, &test_ll, &mock_ll, &mock_ul_for_seed, &mock_ul_for_test}};
    ...
    WHEN("An RFO packet is issued") {
      ...
      AND_WHEN("The seed packet is re-issued") {
        ...
        THEN("The re-issue misses the cache") {
          REQUIRE_THAT(mock_ul_for_seed.packets, Catch::Matchers::SizeIs(2));
          REQUIRE(mock_ul_for_seed.packets.back().return_time - mock_ul_for_seed.packets.back().issue_time == (2*fill_latency + miss_latency + 2*hit_latency + 2)); // +2 due to ordering of elements
        }
      }
    }
  }
