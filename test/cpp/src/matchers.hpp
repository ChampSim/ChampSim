...
struct StrideMatcher : Catch::Matchers::MatcherGenericBase {
  ...
  bool match(const Range& range) const
  {
    ...
    return std::adjacent_find(std::cbegin(range), std::cend(range),
                              [stride = stride](const auto& x, const auto& y) { return champsim::offset(Addr{x}, Addr{y}) != stride; })
           == std::cend(range);
  }

  std::string describe() const override { return "has stride " + std::to_string(stride); }
};
...
struct DisjunctMatcher : Catch::Matchers::MatcherGenericBase {
  ...
  {
    return std::none_of(std::begin(other), std::end(other),
                        [&](const auto other_elem) { return std::find(std::begin(range), std::end(range), other_elem) != std::end(range); });
  }

  std::string describe() const override { return "Contains none of: " + Catch::rangeToString(range); }
...
struct MonotonicallyIncreasingMatcher : Catch::Matchers::MatcherGenericBase {
  ...
  std::string describe() const override { return "Increases monotonically"; }
};
} // namespace champsim::test
