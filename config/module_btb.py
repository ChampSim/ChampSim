...
def get_btb_string(btb_data):
    ...
    retval += '\n    '.join('if (btb_type[lg2(t{})]) {btb_initialize}();'.format(k,**b) for k,b in btb_data.items())
    retval += '\n}\n'
    retval += '\n'
    ...
    retval += '\n    '.join('if (btb_type[lg2(t{})]) {btb_update}(ip, branch_target, taken, branch_type);'.format(k,**b) for k,b in btb_data.items())
    retval += '\n}\n'
    retval += '\n'
    ...
    retval += '\n    '.join('if (btb_type[lg2(t{})]) result = {btb_predict}(ip, branch_type);'.format(k,**b) for k,b in btb_data.items())
    retval += '\n    return result;'
    retval += '\n}\n'
    retval += '\n'
