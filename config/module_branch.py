...
def get_branch_string(branch_data):
    ...
    retval += '\n    '.join('if (bpred_type[lg2(b{})]) {bpred_initialize}();'.format(k,**b) for k,b in branch_data.items())
    retval += '\n}\n'
    retval += '\n'
    ...
    retval += '\n    '.join('if (bpred_type[lg2(b{})]) {bpred_last_result}(ip, target, taken, branch_type);'.format(k,**b) for k,b in branch_data.items())
    retval += '\n}\n'
    retval += '\n'
    ...
    retval += '\n    '.join('if (bpred_type[lg2(b{0})]) result[lg2(b{0})] = {bpred_predict}(ip, predicted_target, always_taken, branch_type);'.format(k,**b) for k,b in branch_data.items())
    retval += '\n    return result.any();'
    retval += '\n}\n\n'
