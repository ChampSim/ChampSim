...
def generate_module_information(containing_dir, module_info):
    ...
    yield os.path.join(containing_dir, 'inc', 'ooo_cpu_module_decl.inc'), core_declarations
    yield os.path.join(containing_dir, 'inc', 'cache_module_decl.inc'), cache_declarations
    yield os.path.join(containing_dir, 'inc', 'module_def.inc'), ('#ifndef GENERATED_MODULES_INC', '#define GENERATED_MODULES_INC', '#include "modules.h"', 'namespace champsim::modules::generated','{')
    yield os.path.join(containing_dir, 'inc', 'module_def.inc'), module_definitions
    yield os.path.join(containing_dir, 'inc', 'module_def.inc'), ('}','#endif')
...
class FileWriter:
    ...
    def write_files(self, parsed_config, bindir_name=None, srcdir_names=None, objdir_name=None, omit_main=False):
        ...
        self.fileparts.extend(generate_module_information(local_objdir_name, module_info))
        ...
        makefile_lines = get_makefile_lines(local_objdir_name, build_id, os.path.normpath(os.path.join(local_bindir_name, executable)), local_srcdir_names, joined_module_info, omit_main)
