...
branch_variant_data = {
        ...
        'bpred_last_result': ('impl_last_branch_result', 'void', '', (('champsim::address', 'ip'), ('champsim::address', 'target'), ('bool', 'taken'), ('uint8_t', 'branch_type'))),
        'bpred_predict': ('impl_predict_branch', 'bool', '|=', (('champsim::address','ip'),))
        }
...
btb_variant_data = {
        ...
        'btb_update': ('impl_update_btb', 'void', '', (('champsim::address','ip'), ('champsim::address','predicted_target'), ('bool','taken'), ('uint8_t','branch_type'))),
        'btb_predict': ('impl_btb_prediction', 'std::pair<champsim::address, bool>', '=', (('champsim::address','ip'),))
        }
...
pref_variant_args = {
    ...
    'prefetcher_branch_operate': ('impl_prefetcher_branch_operate', 'void', '', (('champsim::address', 'ip'), ('uint8_t', 'branch_type'), ('champsim::address', 'branch_target'))),
    'prefetcher_cache_operate': ('impl_prefetcher_cache_operate', 'uint32_t', '^=', (('champsim::address', 'addr'), ('champsim::address', 'ip'), ('uint8_t', 'cache_hit'), ('uint8_t', 'type'), ('uint32_t', 'metadata_in'))),
    'prefetcher_cache_fill': ('impl_prefetcher_cache_fill', 'uint32_t', '^=', (('champsim::address', 'addr'), ('uint32_t', 'set'), ('uint32_t', 'way'), ('uint8_t', 'prefetch'), ('champsim::address', 'evicted_addr'), ('uint32_t', 'metadata_in'))),
...
def get_pref_string(pref_data):
    ...
    retval += '\n'.join('void {prefetcher_branch_operate}(champsim::address, uint8_t, champsim::address) {{ assert(false); }}'.format(**p) for p in pref_data.values() if not p.get('_is_instruction_prefetcher'))
    retval += '\n'
...
repl_variant_data = {
        ...
        'find_victim_func_name': ('impl_replacement_find_victim', 'uint32_t', '=', (('uint32_t','triggering_cpu'), ('uint64_t','instr_id'), ('uint32_t','set'), ('const BLOCK*','current_set'), ('champsim::address','ip'), ('champsim::address','full_addr'), ('uint32_t','type'))),
        'update_func_name': ('impl_replacement_update_state', 'void', '', (('uint32_t','triggering_cpu'), ('uint32_t','set'), ('uint32_t','way'), ('champsim::address','full_addr'), ('champsim::address','ip'), ('champsim::address','victim_addr'), ('uint32_t','type'), ('uint8_t','hit'))),
