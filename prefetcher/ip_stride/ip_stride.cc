...
struct tracker_entry {
  ...
  int64_t last_stride =
      0; // the stride between the last two addresses accessed by this IP
...
void CACHE::prefetcher_cycle_operate() {
  ...
  if (auto [old_pf_address, stride, degree] = lookahead[this]; degree > 0) {
    ...
    if (virtual_prefetch ||
        (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {
      // check the MSHR occupancy to decide if we're going to prefetch to this
      // level or not
      bool success = prefetch_line(
          0, 0, pf_address,
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip,
  ...
  auto found = std::find_if(set_begin, set_end,
                            [ip](tracker_entry x) { return x.ip == ip; });
  ...
  if (found != set_end) {
    ...
    // replace by LRU
    found = std::min_element(set_begin, set_end,
                             [](tracker_entry x, tracker_entry y) {
