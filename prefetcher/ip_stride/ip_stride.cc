...
uint32_t ip_stride::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
...
void ip_stride::prefetcher_cycle_operate()
{
  ...
  if (active_lookahead.has_value()) {
    ...
    if (intern_->virtual_prefetch || (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {
      ...
      bool success = intern_->prefetch_line(pf_address, (intern_->get_occupancy(0, pf_address) < (intern_->get_size(0, pf_address) / 2)), 0);
      if (success)
