...
int L2_ST_update(uint32_t cpu, uint64_t addr)
{
    ...
    if (match == L2_ST_WAY) { // Miss
        ...
        L2_ST_miss[cpu]++; L2_ST_access[cpu]++;
...
int L2_ST_check(uint32_t cpu, uint64_t addr)
{
    ...
    if (warmup_complete[cpu])
    L2_PF_DEBUG(printf("ST_check not found cpu: %d cl_addr: %lx page: %lx block: %ld\n", cpu, addr >> LOG2_BLOCK_SIZE, curr_page, (addr >> LOG2_BLOCK_SIZE) & 0x3F));
    return -1;
}
...
void CACHE::l2c_prefetcher_final_stats()
{
    ...
    for (int i=0; i<L2C_MSHR_SIZE; i++)
        printf("mlc_useful_depth %2d %5.1f%% %10d  mlc_useless_depth %2d %5.1f%% %10d\n", 
        i, (100.0*useful_depth[cpu][i])/temp1, useful_depth[cpu][i], 
        i, (100.0*useless_depth[cpu][i])/temp2, useless_depth[cpu][i]);
    */
}
