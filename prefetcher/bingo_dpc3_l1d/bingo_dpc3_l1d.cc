    ...
    int prefetch(CACHE *cache, uint64_t block_address) {
        if (this->debug_level >= 2) {
            cerr << "PrefetchStreamer::prefetch(cache=" << cache->NAME << ", block_address=0x" << hex << block_address
                 << ")" << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_pq_occupancy().back() << "/" << cache->PQ_SIZE
                 << " PQ entries occupied." << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_mshr_occupancy() << "/" << cache->MSHR_SIZE
        ...
        for (int d = 1; d < this->pattern_len; d += 1) {
            ...
            for (int sgn = +1; sgn >= -1; sgn -= 2) {
                ...
                if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                    ...
                    if (cache->get_pq_occupancy().back() + cache->get_mshr_occupancy() < cache->MSHR_SIZE - 1 &&
                        cache->get_pq_occupancy().back() < cache->PQ_SIZE) {
...
uint32_t CACHE::prefetcher_cache_fill(
    ...
    for (int i = 0; i < static_cast<int>(NUM_CPUS); i += 1)
