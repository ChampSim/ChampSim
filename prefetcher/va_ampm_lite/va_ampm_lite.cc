...
int l2c_prefetch(CACHE* cache, uint64_t ip, uint64_t base_addr, uint64_t pf_addr, bool fill_this_level, int pf_metadata)
{
  ...
  if (cache->prefetch_line(pf_addr, fill_this_level, pf_metadata)) {
    return 1;
  } else {
    return 0;
  }
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  for (std::size_t i = 1; i <= MAX_DISTANCE; i++) {
    if ((::check_cl_access(addr - (i * BLOCK_SIZE))) && (::check_cl_access(addr - (2 * i * BLOCK_SIZE)))
      ...
      bool prefetch_success = (l2c_prefetch(this, ip, addr, addr + (i * BLOCK_SIZE), get_occupancy(0, 0) <= (get_size(0, 0) >> 1), 0) > 0);
      if (prefetch_success) {
