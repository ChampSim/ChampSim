...
l2c_va_ampm_lite_region_t
    l2c_va_ampm_lite_regions[L2C_VA_AMPM_LITE_REGION_COUNT];
uint64_t l2c_va_ampm_lite_region_lru;

int l2c_prefetch(CACHE *cache, uint64_t ip, uint64_t base_addr,
                 uint64_t pf_addr, int pf_fill_level, int pf_metadata) {
  ...
  if (cache->prefetch_line(ip, base_addr, pf_addr, pf_fill_level,
                           pf_metadata)) {
    return 1;
  } else {
    return 0;
...
bool va_ampm_check_access(int region_index, int region_offset) {
  return ((l2c_va_ampm_lite_regions[region_index].access_map) >>
          region_offset) &
         1;
}
...
bool va_ampm_check_prefetch(int region_index, int region_offset) {
  return ((l2c_va_ampm_lite_regions[region_index].prefetch_map) >>
          region_offset) &
         1;
}
...
void va_ampm_reset_prefetch(int region_index, int region_offset) {
  l2c_va_ampm_lite_regions[region_index].prefetch_map &=
      (~(1 << region_offset));
}
...
void CACHE::l2c_prefetcher_initialize() {
  cout << "CPU " << cpu << " L2C Virtual Address Space AMPM-Lite Prefetcher"
       << endl;
...
uint32_t CACHE::l2c_prefetcher_operate(uint64_t addr, uint64_t ip,
  ...
  for (int i = 1; i <= L2C_VA_AMPM_LITE_MAX_DISTANCE; i++) {
    if ((va_ampm_check_cl_access(addr - (i * BLOCK_SIZE))) &&
        (va_ampm_check_cl_access(addr - (2 * i * BLOCK_SIZE))) &&
        (va_ampm_check_cl_access(addr + (i * BLOCK_SIZE)) == false) &&
        (va_ampm_check_cl_prefetch(addr + (i * BLOCK_SIZE)) == false)) {
      ...
      bool prefetch_success =
          (l2c_prefetch(this, ip, addr, addr + (i * BLOCK_SIZE), pf_fill_level,
                        0) > 0);
      if (prefetch_success) {
