...
public:
  ...
  const unsigned pref_activate_mask =
      (1 << static_cast<int>(LOAD)) | (1 << static_cast<int>(PREFETCH));

  // prefetch stats
  uint64_t pf_requested = 0, pf_issued = 0, pf_useful = 0, pf_useless = 0,
           pf_fill = 0;
  ...
  champsim::delay_queue<PACKET> RQ{RQ_SIZE, HIT_LATENCY}, // read queue
      ...
      VAPQ{PQ_SIZE,
           VA_PREFETCH_TRANSLATION_LATENCY}, // virtual address prefetch queue
  ...
  uint64_t RQ_ACCESS = 0, RQ_MERGED = 0, RQ_FULL = 0, RQ_TO_CACHE = 0,
           ...
           WQ_ACCESS = 0, WQ_MERGED = 0, WQ_FULL = 0, WQ_FORWARD = 0,
           WQ_TO_CACHE = 0;
...
#include "cache_modules.inc"
  ...
  // constructor
  CACHE(std::string v1, double freq_scale, unsigned fill_level, uint32_t v2,
      ...
      : champsim::operable(freq_scale), MemoryRequestConsumer(fill_level),
        MemoryRequestProducer(ll), NAME(v1), NUM_SET(v2), NUM_WAY(v3),
        WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8),
        HIT_LATENCY(hit_lat), FILL_LATENCY(fill_lat), OFFSET_BITS(offset_bits),
        MAX_READ(max_read), MAX_WRITE(max_write), prefetch_as_load(pref_load),
        match_offset_bits(wq_full_addr), virtual_prefetch(va_pref),
        pref_activate_mask(pref_act_mask), repl_type(repl), pref_type(pref) {}
};
