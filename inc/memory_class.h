...
public:
  ...
  // stats
  uint64_t ACCESS[NUM_TYPES], HIT[NUM_TYPES], MISS[NUM_TYPES],
      MSHR_MERGED[NUM_TYPES], STALL[NUM_TYPES];
...
class BANK_REQUEST {
public:
  uint64_t cycle_available, address, full_addr;

  uint32_t open_row;

  uint8_t working, working_type, row_buffer_hit, drc_hit, is_write, is_read;
