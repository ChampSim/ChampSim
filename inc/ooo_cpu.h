...
#include "ooo_cpu_module_decl.inc"
  ...
  struct module_concept {
    ...
    virtual void impl_initialize_indirect_branch_predictor() = 0;
    virtual uint64_t impl_predict_indirect_branch(uint64_t ip, uint8_t branch_type) = 0;
    virtual void impl_last_indirect_branch_result(uint64_t ip, uint64_t predicted_target, uint64_t actual_target, uint8_t taken, uint8_t branch_type) = 0;
  ...
  struct module_model final : module_concept {
    ...
    void impl_initialize_indirect_branch_predictor();
    uint64_t impl_predict_indirect_branch(uint64_t ip, uint8_t branch_type);
    void impl_last_indirect_branch_result(uint64_t ip, uint64_t predicted_target, uint64_t actual_target, uint8_t taken, uint8_t branch_type);
  ...
  void impl_initialize_indirect_branch_predictor() { module_pimpl->impl_initialize_indirect_branch_predictor(); }
  void impl_last_indirect_branch_result(uint64_t ip, uint64_t predicted_target, uint64_t actual_target, uint8_t taken, uint8_t branch_type) {
    module_pimpl->impl_last_indirect_branch_result(ip, predicted_target, actual_target, taken, branch_type);
  }
  uint64_t impl_predict_indirect_branch(uint64_t ip, uint8_t branch_type) { return module_pimpl->impl_predict_indirect_branch(ip, branch_type); }
  ...
  public:
    ...
    Builder<A, B_FLAG, T_FLAG> indirect_branch_predictor()
    {
