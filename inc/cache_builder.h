...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::clock_period(champsim::chrono::picoseconds clock_period_) -> self_type&
{
...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::size(champsim::data::bytes size_) -> self_type&
{
  m_size = size_;
  return *this;
}
...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::log2_size(uint64_t log2_size_) -> self_type&
{
...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::log2_ways(uint32_t log2_ways_) -> self_type&
{
  m_ways = 1 << log2_ways_;
  return *this;
}
...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::offset_bits(champsim::data::bits offset_bits_) -> self_type&
{
...
template <typename P, typename R, typename S>
auto champsim::cache_builder<P, R, S>::log2_offset_bits(unsigned log2_offset_bits_) -> self_type&
{
