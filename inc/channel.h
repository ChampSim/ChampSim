...
public:
  ...
  {
    return [req = invalidation_request_type{request}](channel* ul) { ul->invalidation_queue.push_back(req); };
  }

  template <typename T>
  static auto returner_for(T&& request)
  {
    return [req = response_type{request}](channel* ul) { ul->returned.push_back(req); };
  }
};
} // namespace champsim
