...
template <typename T>
using has_last_branch_result_v1 =
    decltype(std::declval<T>().last_branch_result(std::declval<uint64_t>(), std::declval<uint64_t>(), std::declval<bool>(), std::declval<uint8_t>()));

template <typename T>
using has_last_branch_result_v2 = decltype(std::declval<T>().last_branch_result(std::declval<champsim::address>(), std::declval<champsim::address>(),
                                                                                std::declval<bool>(), std::declval<uint8_t>()));
...
template <typename T>
using has_predict_branch_v3 =
    decltype(std::declval<T>().predict_branch(std::declval<uint64_t>(), std::declval<uint64_t>(), std::declval<bool>(), std::declval<uint8_t>()));

/* Strong addresses, full size */
template <typename T>
using has_predict_branch_v4 = decltype(std::declval<T>().predict_branch(std::declval<champsim::address>(), std::declval<champsim::address>(),
                                                                        std::declval<bool>(), std::declval<uint8_t>()));
} // namespace detail
...
{
  if (champsim::is_detected_v<detail::has_predict_branch_v4, T>)
    return 4;
  if (champsim::is_detected_v<detail::has_predict_branch_v3, T>)
    return 3;
...
template <typename T>
using has_update_btb_v1 =
    decltype(std::declval<T>().update_btb(std::declval<uint64_t>(), std::declval<uint64_t>(), std::declval<bool>(), std::declval<uint8_t>()));

template <typename T>
using has_update_btb_v2 =
    decltype(std::declval<T>().update_btb(std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<bool>(), std::declval<uint8_t>()));
...
/* Raw integer addresses, full size */
template <typename T>
using has_btb_prediction_v3 = decltype(std::declval<T>().btb_prediction(std::declval<uint64_t>(), std::declval<uint8_t>()));

/* Strong addresses, full size */
template <typename T>
using has_btb_prediction_v4 = decltype(std::declval<T>().btb_prediction(std::declval<champsim::address>(), std::declval<uint8_t>()));
} // namespace detail
...
{
  if (champsim::is_detected_v<detail::has_btb_prediction_v4, T>)
    return 4;
  if (champsim::is_detected_v<detail::has_btb_prediction_v3, T>)
    return 3;
...
template <typename T>
using has_cache_operate_v2 =
    decltype(std::declval<T>().prefetcher_cache_operate(std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<uint8_t>(),
                                                        std::declval<bool>(), std::declval<uint8_t>(), std::declval<uint32_t>()));

/* Strong addresses */
template <typename T>
using has_cache_operate_v3 =
    decltype(std::declval<T>().prefetcher_cache_operate(std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<uint8_t>(),
                                                        std::declval<bool>(), std::declval<access_type>(), std::declval<uint32_t>()));

template <typename T>
using has_cache_fill_v1 = decltype(std::declval<T>().prefetcher_cache_fill(std::declval<uint64_t>(), std::declval<long>(), std::declval<long>(),
                                                                           std::declval<uint8_t>(), std::declval<uint64_t>(), std::declval<uint32_t>()));

template <typename T>
using has_cache_fill_v2 =
    decltype(std::declval<T>().prefetcher_cache_fill(std::declval<champsim::address>(), std::declval<long>(), std::declval<long>(), std::declval<uint8_t>(),
                                                     std::declval<champsim::address>(), std::declval<uint32_t>()));
...
template <typename T>
using has_branch_operate_v1 =
    decltype(std::declval<T>().prefetcher_branch_operate(std::declval<uint64_t>(), std::declval<uint8_t>(), std::declval<uint64_t>()));

template <typename T>
using has_branch_operate_v2 =
    decltype(std::declval<T>().prefetcher_branch_operate(std::declval<champsim::address>(), std::declval<uint8_t>(), std::declval<champsim::address>()));
} // namespace detail
...
using has_find_victim_v2 =
    decltype(std::declval<T>().find_victim(std::declval<uint32_t>(), std::declval<uint64_t>(), std::declval<long>(), std::declval<champsim::cache_block*>(),
                                           std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<uint32_t>()));

/* Strong addresses */
template <typename T>
using has_find_victim_v3 =
    decltype(std::declval<T>().find_victim(std::declval<uint32_t>(), std::declval<uint64_t>(), std::declval<long>(), std::declval<champsim::cache_block*>(),
                                           std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<access_type>()));
...
/* Raw integer access type */
template <typename T>
using has_update_state_v2 =
    decltype(std::declval<T>().update_replacement_state(std::declval<uint32_t>(), std::declval<long>(), std::declval<long>(), std::declval<champsim::address>(),
                                                        std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<uint32_t>(),
                                                        std::declval<uint8_t>()));

/* Strong addresses */
template <typename T>
using has_update_state_v3 =
    decltype(std::declval<T>().update_replacement_state(std::declval<uint32_t>(), std::declval<long>(), std::declval<long>(), std::declval<champsim::address>(),
                                                        std::declval<champsim::address>(), std::declval<champsim::address>(), std::declval<access_type>(),
                                                        std::declval<uint8_t>()));
...
{
  if (champsim::is_detected_v<detail::has_find_victim_v3, T>)
    return 3;
...
{
  if (champsim::is_detected_v<detail::has_update_state_v3, T>)
    return 3;
